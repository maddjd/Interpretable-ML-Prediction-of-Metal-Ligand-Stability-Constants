...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "MLP_IUPAC_Final_Results")

TRAIN_CSV = "MLP_Train_Results.csv"
TEST_CSV = "MLP_Test_Results.csv"
AD_CSV = "MLP_AD_Data.csv"
...
HIDDEN_DIMS = [66, 89, 16]
DROPOUT_RATE = 0.0153400786476779
LEARNING_RATE = 0.000179128627611025
WEIGHT_DECAY = 0.00798588905973722
BATCH_SIZE = 64
NUM_EPOCHS = 1000
MODEL_SEED = 42
...
class ImprovedMLP(nn.Cell):
    def __init__(self, input_dim, hidden_dims, output_dim, dropout_rate):
        ...
        for hidden_dim in hidden_dims:
            layers.append(
                ResidualBlock(previous_dim, hidden_dim, dropout_rate)
            )
...
class EpochLossRecorder(Callback):
    ...
    def on_train_epoch_end(self, run_context):
        ...
        if self.x_validation is not None:
            ...
            prediction = (
                self.model_ms.predict(self.x_validation)
                .asnumpy()
                .reshape(-1)
            )
            self.network.set_train(True)
            validation_loss = float(
                mean_squared_error(self.y_validation, prediction)
            )
        ...
        if epoch == 1 or epoch % self.print_every == 0 or epoch == self.total_epochs:
            if validation_loss is None:
                print(
                    ...
                    f"training MSE loss = {train_loss:.8f}"
                )
            else:
                print(
                    f"Epoch {epoch:4d}/{self.total_epochs} | "
...
def train_one_model(
    ...
    train_prediction = (
        model_ms.predict(Tensor(x_train, ms.float32))
        .asnumpy()
        .reshape(-1)
    )
    predict_prediction = (
        model_ms.predict(Tensor(x_predict, ms.float32))
        .asnumpy()
        .reshape(-1)
    )
    ...
    return {
        ...
        "train_losses": recorder.train_losses,
        "validation_losses": recorder.validation_losses,
    }
...
def applicability_domain_analysis(
    ...
    train_std_residual = train_residual / residual_std
    test_std_residual = test_residual / residual_std

    train_distance = np.mean(
        cdist(x_train, x_train),
        axis=1
    )

    test_distance = np.mean(
        cdist(x_test, x_train),
        axis=1
    )
    ...
    train_df = pd.DataFrame({
        ...
        "Predicted_logK": pred_train,
        "Residual": train_residual,
        "Average_Feature_Distance": train_distance,
        "Standardized_Residual": train_std_residual,
    })
    ...
    test_df = pd.DataFrame({
        ...
        "Predicted_logK": pred_test,
        "Residual": test_residual,
        "Average_Feature_Distance": test_distance,
        "Standardized_Residual": test_std_residual,
    })
...
def build_prediction_results(
    ...
    return pd.DataFrame({
        "Split": split_name,
        "Filtered_Row_Index": np.asarray(indices, dtype=int),
        "Original_Row_Index": original_row_indices[indices],
...
def main():
    ...
    if TARGET_COLUMN not in meta_raw.columns:
        raise KeyError(f" '{TARGET_COLUMN}'。")
    ...
    train80_indices, test20_indices = train_test_split(
        all_indices,
        test_size=TEST_SIZE,
    ...
    print(f"：{len(y)}")
    print("： 80% + 20%")
    print("：80% KFold(n_splits=5)")
    print("：； groups ")
    print("：；1000")
    print(f"80%：{len(train80_indices)}")
    print(f"20%：{len(test20_indices)}")
    ...
    for fold_number, (fold_train_local, fold_valid_local) in enumerate(
        ...
        print(f"：{x_fold_train.shape[1]}")

        result = train_one_model(
            x_train=x_fold_train,
            y_train=y[fold_train_indices],
            x_predict=x_fold_valid,
            seed=MODEL_SEED + fold_number,
            model_tag=f"internal_fold_{fold_number}",
            x_validation=x_fold_valid,
            y_validation=y[fold_valid_indices],
        )
        ...
        row = {
            ...
            "n_validation": len(fold_valid_indices),
        }
    ...
    x_train80 = final_preprocessor.fit_transform(
        feature_df.iloc[train80_indices]
    )
    x_test20 = final_preprocessor.transform(
        feature_df.iloc[test20_indices]
    )

    print(f"：{x_train80.shape[1]}")

    final_result = train_one_model(
        x_train=x_train80,
        y_train=y[train80_indices],
        x_predict=x_test20,
        seed=MODEL_SEED,
        model_tag="final_train80",
        x_validation=None,
        y_validation=None,
    )
    ...
    train_results = build_prediction_results(
        indices=train80_indices,
        original_row_indices=original_row_indices,
        y_true=y[train80_indices],
        y_pred=final_train_prediction,
        split_name="Training",
    ...
    test_results = build_prediction_results(
        indices=test20_indices,
        original_row_indices=original_row_indices,
        y_true=y[test20_indices],
    ...
    ad_data = applicability_domain_analysis(
        x_train=x_train80,
        x_test=x_test20,
        y_train=y[train80_indices],
        pred_train=final_train_prediction,
        y_test=y[test20_indices],
        pred_test=final_test_prediction,
        train_indices=train80_indices,
        test_indices=test20_indices,
        original_row_indices=original_row_indices,
    )
    ad_data.to_csv(
    ...
    print("\n5：")
    print(
        f"R²={fold_metrics_df['train_R2'].mean():.6f} ± "
        f"{fold_metrics_df['train_R2'].std(ddof=1):.6f}, "
        f"MAE={fold_metrics_df['train_MAE'].mean():.6f} ± "
        f"{fold_metrics_df['train_MAE'].std(ddof=1):.6f}, "
        f"MSE={fold_metrics_df['train_MSE'].mean():.6f} ± "
        f"{fold_metrics_df['train_MSE'].std(ddof=1):.6f}, "
        f"RMSE={fold_metrics_df['train_RMSE'].mean():.6f} ± "
        f"{fold_metrics_df['train_RMSE'].std(ddof=1):.6f}"
    )

    print("5：")
    print(
        f"R²={fold_metrics_df['validation_R2'].mean():.6f} ± "
        f"{fold_metrics_df['validation_R2'].std(ddof=1):.6f}, "
        f"MAE={fold_metrics_df['validation_MAE'].mean():.6f} ± "
        f"{fold_metrics_df['validation_MAE'].std(ddof=1):.6f}, "
        f"MSE={fold_metrics_df['validation_MSE'].mean():.6f} ± "
        f"{fold_metrics_df['validation_MSE'].std(ddof=1):.6f}, "
        f"RMSE={fold_metrics_df['validation_RMSE'].mean():.6f} ± "
        f"{fold_metrics_df['validation_RMSE'].std(ddof=1):.6f}"
    )
    ...
    print(
        f"\nTraining AD residual outliers: "
        f">+3: {train_above}, <-3: {train_below}"
    )
    print(
        f"Testing AD residual outliers: "
