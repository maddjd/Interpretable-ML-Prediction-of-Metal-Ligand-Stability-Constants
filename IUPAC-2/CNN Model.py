...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "CNN_IUPAC_Final_Results")
...
best_params = {
    "learning_rate": 0.000147423522859472,
    "dropout_rate": 0.0243809895267736,
    "weight_decay": 1.71953627754313E-06,
    "batch_size": 16,
    "out_channels": 93,
    "kernel_size": 4,
    "fc1_units": 109,
...
def main():
    ...
    for fold_number, (fold_train_local, fold_valid_local) in enumerate(
        ...
        fold_train_loss_curves.append(
            np.asarray(fold_train_losses, dtype=float)
        )
        fold_validation_loss_curves.append(
            np.asarray(fold_valid_losses, dtype=float)
        )
    ...
    mean_train_loss = np.mean(
        np.vstack(fold_train_loss_curves),
        axis=0,
    )
    mean_validation_loss = np.mean(
        np.vstack(fold_validation_loss_curves),
        axis=0,
    )
